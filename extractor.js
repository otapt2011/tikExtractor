...
(function(global) {
  ...
  const DEFAULT_PATHS = {
    dm:        ["Direct Message", "Direct Messages", "ChatHistory"],
  ...
  let fieldMappings = {
    dm:      { date: "Date", from: "From", content: "Content", to: "To" },
  ...
  let itemMappers = {
    dm: null, comment: null, post: null,
  ...
  function extractDm(data) {
    const chatHistory = safeGetNested(data, currentPaths.dm, "DM");
    if (!chatHistory) return [];
    const allMessages = [];
    for (const [key, messages] of Object.entries(chatHistory)) {
      const prefix = "Chat History with ";
      if (!key.startsWith(prefix)) continue;
      const participant = key.slice(prefix.length);
      if (!Array.isArray(messages)) continue;
      for (const msg of messages) {
        let enhanced = applyMapping(msg, fieldMappings.dm);
        const from = msg.From || enhanced.From;
        const to = (from === participant) ? currentUser : participant;
        allMessages.push({ ...enhanced, To: to });
      }
    }
    if (itemMappers.dm) return allMessages.map(itemMappers.dm);
    return allMessages;
  }
  ...
  const TikTokExtractor = {
    extractDm, extractComment, extractPost, extractFollowing,
