...
self.addEventListener('message', async function(e) {
  ...
  try {
    ...
    if (finalUser) {
      extractor.setCurrentUser(finalUser);
    } else {
      extractor.setCurrentUser("");
    }
    const result = {};
    ...
    const followingTotal = (result.following?.length || 0) + (followingNA || 0);
const followerTotal = (result.follower?.length || 0) + (followerNA || 0);

const validation = {
  following: {
    extracted: result.following?.length || 0,
    profile: profile.followingCount || 0,
    match: followingTotal === (profile.followingCount || 0)
  },
  follower: {
    extracted: result.follower?.length || 0,
    profile: profile.followerCount || 0,
    match: followerTotal === (profile.followerCount || 0)
  },
  likes: {
    extracted: totalLikes,
    profile: profile.likesReceived || 0,
    match: totalLikes === (profile.likesReceived || 0)
  },
