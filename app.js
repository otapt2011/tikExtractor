...
(function(global) {
  ...
  static get defaults() {
    return [
      ...
      { id: 'friends', label: 'Friends', icon: 'handshake', iconColor: 'text-gold-500 dark:text-gold-300', defaultPath: null, isComputed: true, dependsOn: ['following', 'follower'] },
      ...
      { id: 'messages', label: 'Messages', icon: 'comment-dots', iconColor: 'text-brand-500 dark:text-teal-400', defaultPath: ['Direct Message', 'Direct Messages', 'ChatHistory'], isComputed: false, needsCurrentUser: true },
      ...
      { id: 'repost', label: 'Repost', icon: 'retweet', iconColor: 'text-green-600 dark:text-green-400', defaultPath: ['Your Activity', 'Reposts', 'RepostList'], isComputed: false, dependsOn: [], needsCurrentUser: false },
      { id: 'searches', label: 'Searches', icon: 'search', iconColor: 'text-blue-500 dark:text-blue-400', defaultPath: ['Your Activity', 'Searches', 'SearchList'], isComputed: false, dependsOn: [], needsCurrentUser: false },
      { id: 'hashtag', label: 'Hashtag', icon: 'hashtag', iconColor: 'text-pink-500 dark:text-pink-400', defaultPath: ['Your Activity', 'Hashtag', 'HashtagList'], isComputed: false, dependsOn: [], needsCurrentUser: false },
      { id: 'share', label: 'Share', icon: 'share-alt', iconColor: 'text-emerald-500 dark:text-emerald-400', defaultPath: ['Your Activity', 'Share History', 'ShareHistoryList'], isComputed: false, dependsOn: [], needsCurrentUser: false },
      { id: 'stickers', label: 'Stickers', icon: 'smile', iconColor: 'text-yellow-500 dark:text-yellow-400', defaultPath: ['Your Activity', 'Stickers', 'StickerList'], isComputed: false, dependsOn: [], needsCurrentUser: false },
      { id: 'liked', label: 'Liked', icon: 'heart', iconColor: 'text-red-500 dark:text-red-400', defaultPath: ['Likes and Favorites', 'Like List', 'ItemFavoriteList'], isComputed: false, dependsOn: [], needsCurrentUser: false }
    ];
  }
  ...
  class DataShare {
    ...
    static extractArrayInfo(data = null) {
  const target = data || window.App.getExtracted();
  if (!target) {
    console.warn('No data available to extract array info');
    return {};
  }
  const result = {};
  ...
  function traverse(current, pathParts) {
    ...
    if (Array.isArray(current)) {
      ...
      result[path] = {
        ...
        fields: hasObjects ? Array.from(fieldsSet).sort() : []
      };
      // Recurse into array items to find nested arrays
      for (const item of current) {
        if (item && typeof item === 'object') traverse(item, pathParts);
      }
      return;
    }
...
class DataTransformer {
  static get mappings() {
    return {
      ...
      following: { UserName: 'user_name', Date: 'date' },
      follower: { UserName: 'user_name', Date: 'date' },
      blocked: { UserName: 'user_name', Date: 'date' },
      friends: { UserName: 'user_name', FollowingDate: 'following_date', FollowerDate: 'follower_date' },
      messages: { date: 'message_date', from: 'from_user', content: 'content', to: 'to_user' },
      comment: { date: 'comment_date', comment: 'comment_text', photo: 'photo_url', url: 'video_url' },
      post: { date: 'post_date', likes: 'likes_count', coverImage: 'cover_image' },
      repost: { Date: 'repost_date', Link: 'repost_link' },
      searches: { Date: 'search_date', SearchTerm: 'search_term' },
      hashtag: { HashtagLink: 'hashtag_link', HashtagName: 'hashtag_name' },
      share: { Date: 'share_date', Link: 'share_link', Method: 'share_method', SharedContent: 'shared_content' },
      stickers: { CreationTime: 'creation_time', Link: 'sticker_link' },
      liked: { date: 'liked_date', link: 'liked_link' }
    };
  }
  
  static transformObject(obj, mapping) {
    if (!obj || typeof obj !== 'object') return {};
    const result = {};
    for (const [orig, newKey] of Object.entries(mapping)) {
      if (orig in obj && obj[orig] !== undefined && obj[orig] !== null) {
        result[newKey] = obj[orig];
      }
    }
    return result;
  }
  
  static transform(extracted) {
  if (!extracted) return null;
  const mappings = this.mappings;
  const result = {};
  
  // Profile (single object)
  if (extracted.profile && extracted.profile.length > 0) {
    result.profile = this.transformObject(extracted.profile[0], mappings.profile);
  } else {
    result.profile = null;
  }
  
  // Array types
  const arrayTypes = ['following', 'follower', 'blocked', 'friends', 'messages', 'comment', 'post', 'repost', 'searches', 'hashtag', 'share', 'stickers', 'liked'];
  for (const type of arrayTypes) {
    const mapping = mappings[type];
    if (mapping && Array.isArray(extracted[type])) {
      result[type] = extracted[type].map(item => this.transformObject(item, mapping));
    } else {
      result[type] = [];
    }
  }
      ...
      this.worker.onmessage = (e) => {
        ...
        if (msg.type === 'progress') {
          ...
          if (msg.success) {
            ...
            if (this.extracted.currentUserUsed) {
              this.currentUser = this.extracted.currentUserUsed;
            }
            this.emitUpdate();
          } else {
...
appInstance.Transform = DataTransformer;
  ...
  global.Transform = DataTransformer;
  global.safeStringify = safeStringify;
